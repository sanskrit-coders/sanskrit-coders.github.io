...
function appendTool() {
    ...
    // Selection Items
    var selectInit = '<span class="aksharamukha-selection"><select name="scriptinput" id="aksharamukhaselect"><option disabled selected value> -- select a script -- </option>'
...
` + selectInit + selectMid + selectEnd + preservebutton + `
  ...
  `));
    ...
    link.type = "text/css";
    link.rel = "stylesheet";
    link.href = 'http://www.virtualvinodh.com/aksharamkh/fonts.css'
    ...
    var transContent = document.getElementsByClassName('aksharamukha-text')
...
function transliterate() {
    ...
    for (var i = 0; i < transContent.length; i++) {
        ...
        getResult(transContent[i], i, source, target)
    }
}
...
function getResult(el, i, source, target) {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            el.classList.add(ScriptMixin.methods.getOutputClass(target, postOptionsList))
    ...
    xhttp.open("POST", "http://aksharamukha.appspot.com/api/plugin", true)
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded")
    xhttp.send("source=" + source + "&target=" + target + "&text=" + origText[i] + "&postOptions=" + JSON.stringify(postOptionsList) + "&nativize=" + JSON.stringify(!preservePrevious))
}
